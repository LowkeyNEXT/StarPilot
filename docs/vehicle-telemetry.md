...
- `chargingTimeRemaining` in seconds
...
The transport converts `chargingTimeRemaining` to integer `minutesToFull`, which
matches the external vehicle-telemetry contract while keeping the vehicle schema
in SI units.

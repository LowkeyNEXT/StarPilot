...
class TestCanFingerprint:
  ...
  def test_fingerprint_default_still_queries_vin_and_firmware(self, monkeypatch):
    monkeypatch.delenv("FINGERPRINT", raising=False)
    monkeypatch.delenv("SKIP_FW_QUERY", raising=False)
    monkeypatch.delenv("DISABLE_FW_CACHE", raising=False)
    query_calls = []
    monkeypatch.setattr(car_helpers, "get_vin", lambda *_args, **_kwargs: (-1, -1, "KM8KNDAF0PU123456"))
    monkeypatch.setattr(car_helpers, "get_present_ecus",
                        lambda *_args, **_kwargs: query_calls.append("ecus") or set())
    monkeypatch.setattr(car_helpers, "get_fw_versions_ordered",
                        lambda *_args, **_kwargs: query_calls.append("firmware") or [])
    monkeypatch.setattr(car_helpers, "match_fw_to_car", lambda _car_fw, _vin: (True, set()))
    monkeypatch.setattr(car_helpers, "can_fingerprint", lambda _can_recv: (None, {0: {}, 1: {}, 2: {}, 3: {}}))

    set_obd_calls = []
    fingerprint_car(lambda **kwargs: [], lambda _msgs: None, set_obd_calls.append, 1, None)

    assert query_calls == ["ecus", "firmware"]
    assert set_obd_calls == [True, False]

...
# Public compatibility exports for callers that imported these helpers before
# their implementation moved into EV9-owned modules.
from opendbc.car.hyundai.ev9_bsm import (BLINDSPOT_WARNING_FLASH_ON_SAMPLES as BLINDSPOT_WARNING_FLASH_ON_SAMPLES,
                                         BLINDSPOT_WARNING_FLASH_SAMPLES as BLINDSPOT_WARNING_FLASH_SAMPLES,
                                         BLINDSPOT_WARNING_SOUND_SAMPLES as BLINDSPOT_WARNING_SOUND_SAMPLES,
                                         BlindspotWarningOutput as BlindspotWarningOutput,
...
from opendbc.car.hyundai.ev9_controller import EV9Controller, ev9_dynamic_steering_icons as ev9_dynamic_steering_icons, \
                                                  ev9_reconstructed_steering_available as ev9_reconstructed_steering_available
from opendbc.car.hyundai.ev9_longitudinal import (EV9_STANDSTILL_DELAY_FRAMES as EV9_STANDSTILL_DELAY_FRAMES,
                                                  EV9_STOP_RELEASE_DELAY_FRAMES as EV9_STOP_RELEASE_DELAY_FRAMES,
                                                  EV9_STOP_REQUEST_SPEED as EV9_STOP_REQUEST_SPEED,
                                                  EV9LongitudinalTuningState as EV9LongitudinalTuningState,
                                                  update_ev9_longitudinal_tuning as update_ev9_longitudinal_tuning)
...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP):
    ...
    self._params = Params()
    self.ev9 = EV9Controller(CP, self.packer, self.CAN)
  ...
  def update(self, CC, CS, now_nanos, starpilot_toggles):
    ...
    if self.CP.flags & HyundaiFlags.CANFD_ANGLE_STEERING:
      ...
      lka_icon, lfa_icon, ev9_ccnc_steering_active = ev9_dynamic_steering_icons(
        ...
        lka_icon, lfa_icon, self.ev9.direct_path_available(CC, CS, self.long_active_ecu),
      )
    else:
      # steering torque
    ...
    is_ev9 = self.ev9.enabled
    if is_ev9 and (not CC.enabled or CC.cruiseControl.override):
  ...
  def create_canfd_msgs(self, now_nanos, apply_steer_req, apply_torque, apply_angle, set_speed_in_units, accel, stopping,
    ...
    ev9_long_active = self.ev9.active(self.long_active_ecu)
    ev9_main_mode = self.ev9.main_mode(CS)
    use_egmp_dynamic_long_tuning = egmp_dynamic_longitudinal_tuning(self.CP) and self.long_active_ecu and \
    ...
    apply_torque, apply_angle = self.ev9.inhibit_faulted_steering(
      CS, self.long_active_ecu, apply_torque, apply_angle,
    )

    gear = getattr(getattr(CS, "out", None), "gearShifter", None)
    drive_gear = gear == structs.CarState.GearShifter.drive
    ev9_actuation_permitted = self.ev9.actuation_permitted(self.frame, CC, CS, accel) if ev9_long_active else True
    if angle_lkas_alt:
    ...
    # Match Panda's ownership decision at standstill: the EV9 angle-long
    # profile remains host-owned, while every other angle profile forwards the
    # stock LKAS_ALT pair and rejects the host camera companion.
    if bool(getattr(CS.out, "standstill", False)) and not self.CP.steerAtStandstill:
    ...
    if ev9_direct_angle_mode:
      can_sends.extend(self.ev9.create_steering_messages(
        self.frame, CC, CS, apply_torque, apply_angle, ev9_actuation_permitted,
      ))
    ...
    if self.frame % 5 == 0 and (not lka_steering or lka_steering_long):
      if ev9_long_active:
        can_sends.extend(self.ev9.create_dash_messages(
          self.frame, CC, CS, ev9_main_mode, ev9_ccnc_steering_active, ev9_actuation_permitted,
        ))
      elif ccnc_non_hda2:
    ...
    if self.long_active_ecu:
      if lka_steering:
        if ev9_long_active:
          can_sends.extend(self.ev9.create_adrv_messages(self.frame, now_nanos, CS))
        else:
          can_sends.extend(hyundaicanfd.create_adrv_messages(self.packer, self.CAN, self.frame))
          if self.CP.carFingerprint in CANFD_RADAR_LIVE_LONGITUDINAL_CAR and self.frame % 4 == 0:
            can_sends.append(hyundaicanfd.create_accelerator_brake_alt_spoof(
              0, self.frame // 4, CS.out.brakePressed, CS.out.gasPressed,
            ))
      ...
      if self.frame % 2 == 0:
        ...
        if ev9_long_active:
          acc_control, self.accel_last = self.ev9.create_acc_control(
            CC, CS, self.accel_last, accel, stopping, set_speed_in_units, ev9_main_mode,
            lead_visible, lead_distance, lead_rel_speed, ev9_actuation_permitted,
          )
          can_sends.append(acc_control)
        else:
          acc_kwargs = {
            "main_mode_acc": int(CS.out.cruiseState.available),
            "direct_accel": True,
            "jerk_lower": self._ioniq_6_long_tuning.jerk_lower if use_egmp_smoothed_accel else 5.0,
            "jerk_upper": self._ioniq_6_long_tuning.jerk_upper if use_egmp_smoothed_accel else
              3.0 if CC.actuators.longControlState == LongCtrlState.pid else 1.0,

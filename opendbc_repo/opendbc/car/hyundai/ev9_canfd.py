...
from opendbc.car.hyundai.hyundaicanfd import _create_angle_adas_cmd_msg, _set_ccnc_message_signals, \
                                                hkg_can_fd_checksum
...
# ADAS_DRV. Reset these on every pre-fingerprint attempt.
...
def create_inactive_steering_messages(packer, CAN, steering_angle: float, counter: int = 0):
  lfa_values = {
...
def create_acc_control(packer, CAN, counter: int, enabled: bool, accel_raw: float, accel_value: float,
  ...
  values = {
    ...
    "MainMode_ACC": int(bool(main_mode_acc)), "StopReq": 1 if stop_request and enabled else 0,
    ...
    "ObjValid": 0 if lead_visible else 1, "OBJ_STATUS": 2 if enabled and lead_visible else 0,

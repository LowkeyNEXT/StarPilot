...
from opendbc.car.disable_ecu import ecu_log
...
def ev9_preinit_enabled(CP, params) -> bool:
  """Return whether this exact interface may use the alternate startup path."""
  return bool(CP.carFingerprint == CAR.KIA_EV9 and
              CP.openpilotLongitudinalControl and
              ev9_panda_preinit_armed(params))
...
def ev9_interface_init_profile(CP, params, normal_init: bool) -> EV9InterfaceInitProfile:
  ...
  enabled = ev9_preinit_enabled(CP, params)
  if not enabled:
  ...
  return EV9InterfaceInitProfile(
    ...
    handoff_adopted=resume_panda,
    host_request_veto=not resume_panda and normal_init and transaction_owned,
...
def attempt_ev9_pre_fingerprint_suppression(cached_params, params, initial_can_messages: list | None = None) -> bool:
  """Adopt verified resident Panda ownership before fingerprinting."""
  ev9_preinit_canfd.set_adrv_baselines([])
  if not params.get_bool("EV9LongPreinitPanda"):
    return False
  ...
  if ev9_panda_preinit_armed(params) and handoff.adoptable:
    ev9_preinit_canfd.set_adrv_baselines(ev9_panda_preinit_baselines(observed_can_messages))

...
@pytest.mark.parametrize(("uds_request", "cc_response", "require_positive_response", "expected"), (
...
def test_interface_profile_requires_all_persistent_gates_and_exact_platform():
  enabled = {
  ...
  CP = SimpleNamespace(carFingerprint=CAR.KIA_EV9, openpilotLongitudinalControl=True)
  assert ev9_interface_init_profile(CP, FakeParams(enabled), normal_init=True).enabled

  for key in enabled:
    assert not ev9_interface_init_profile(CP, FakeParams(enabled | {key: False}), normal_init=True).enabled

  other_car = SimpleNamespace(carFingerprint=CAR.HYUNDAI_IONIQ_6, openpilotLongitudinalControl=True)
  assert not ev9_interface_init_profile(other_car, FakeParams(enabled), normal_init=True).enabled
...
def test_upstream_helpers_remain_in_their_original_modules():
  ...
  assert tuple(inspect.signature(carcontroller.CarController._get_canfd_scc_lead_state).parameters) == \
    ("self", "CC", "CS", "now_nanos")

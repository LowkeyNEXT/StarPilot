...
class TestHyundaiFingerprint:
  ...
  def test_ev9_stopped_driver_override_release_holds_temporarily(self):
    ...
    assert not carcontroller.ev9_driver_override_release_active(ev9_cp, True, False, 120, 100, 13.6)
  ...
  def test_ev9_angle_status_stays_active_when_gain_is_zero(self):
    ...
    cc = SimpleNamespace(enabled=True, latActive=True, alwaysOnLateralEnabled=True,
  ...
  def test_ev9_enabled_inactive_angle_steering_lets_safety_forward_stock_lkas(self):
    CP = CarParams.new_message()
    CP.carFingerprint = CAR.KIA_EV9
    CP.flags = int(HyundaiFlags.CANFD | HyundaiFlags.EV | HyundaiFlags.CANFD_ANGLE_STEERING |
                   HyundaiFlags.CANFD_LKA_STEERING | HyundaiFlags.CANFD_LKA_STEERING_ALT)
  ...
  def test_ev9_active_angle_steering_sets_ccnc_lfa_icon_green(self):
    CP = CarParams.new_message()
    CP.carFingerprint = CAR.KIA_EV9
    CP.flags = int(HyundaiFlags.CANFD | HyundaiFlags.EV | HyundaiFlags.CANFD_ANGLE_STEERING |
    ...
    cs = SimpleNamespace(stock_lfa_msg=None, stock_lkas_msg={}, lfa_block_msg=lfa_block_msg, msg_161=msg_161,
                         out=SimpleNamespace(steeringAngleDeg=0.0, gearShifter=structs.CarState.GearShifter.drive))

    msgs = controller.create_canfd_msgs(0, True, 0.44, 0.0, 0.0, 0.0, False, cc.hudControl, cs, cc,
                                        get_test_toggles(), lka_icon=2, lfa_icon=2)
  ...
  def test_ev9_aol_enabled_but_inactive_sets_ccnc_lfa_icon_gray(self):
    CP = CarParams.new_message()
    CP.carFingerprint = CAR.KIA_EV9
    CP.flags = int(HyundaiFlags.CANFD | HyundaiFlags.EV | HyundaiFlags.CANFD_ANGLE_STEERING |
                   HyundaiFlags.CANFD_LKA_STEERING | HyundaiFlags.CANFD_LKA_STEERING_ALT | HyundaiFlags.CCNC)
    ...
    can_bus = CanBus(CP)
    parser = CANParser(DBC[CP.carFingerprint][Bus.pt], [("CCNC_0x161", 0)], can_bus.ECAN)
    cc = SimpleNamespace(enabled=False, latActive=False, alwaysOnLateralEnabled=True,
                         actuators=SimpleNamespace(longControlState=LongCtrlState.off),
                         leftBlinker=False, rightBlinker=False, hudControl=SimpleNamespace())
    lfa_block_msg = {f"BYTE{i}": 0 for i in range(3, 32) if i != 7}
    lfa_block_msg["COUNTER"] = 0
  ...
  def test_ev9_acc_enabled_aol_off_without_recent_aol_does_not_override_ccnc_lfa_icon(self):
    ...
    controller = CarController(DBC[CP.carFingerprint], CP)
    controller.frame = 5
    cc = SimpleNamespace(enabled=True, latActive=False, alwaysOnLateralEnabled=False,
                         actuators=SimpleNamespace(longControlState=LongCtrlState.off),
                         leftBlinker=False, rightBlinker=False, hudControl=SimpleNamespace())
    lfa_block_msg = {f"BYTE{i}": 0 for i in range(3, 32) if i != 7}
    lfa_block_msg["COUNTER"] = 0
  ...
  def test_ev9_aol_enabled_non_drive_sets_ccnc_lfa_icon_gray(self, gear):
    CP = CarParams.new_message()
    CP.carFingerprint = CAR.KIA_EV9
    CP.flags = int(HyundaiFlags.CANFD | HyundaiFlags.EV | HyundaiFlags.CANFD_ANGLE_STEERING |
                   HyundaiFlags.CANFD_LKA_STEERING | HyundaiFlags.CANFD_LKA_STEERING_ALT | HyundaiFlags.CCNC)
    ...
    lfa_block_msg["COUNTER"] = 0
    cs = SimpleNamespace(stock_lfa_msg=None, stock_lkas_msg={}, lfa_block_msg=lfa_block_msg,
                         msg_161={"LFA_ICON": 0, "LKA_ICON": 3, "ALERTS_2": 9, "SOUNDS_4": 2},
                         out=SimpleNamespace(steeringAngleDeg=0.0, gearShifter=gear))
  ...
  def test_ev9_driver_torque_without_pressed_keeps_angle_command(self):
    CP = CarParams.new_message()
    CP.carFingerprint = CAR.KIA_EV9
    CP.flags = int(HyundaiFlags.CANFD | HyundaiFlags.EV | HyundaiFlags.CANFD_ANGLE_STEERING |
                   HyundaiFlags.CANFD_LKA_STEERING | HyundaiFlags.CANFD_LKA_STEERING_ALT)
    ...
    cs = SimpleNamespace(stock_lfa_msg=None, stock_lkas_msg=stock_lkas, lfa_block_msg=lfa_block_msg,
                         out=SimpleNamespace(steeringAngleDeg=-40.8, steeringTorque=-180.0, steeringPressed=False,
                                             gearShifter=structs.CarState.GearShifter.drive))

    msgs = controller.create_canfd_msgs(0, True, 0.20, -46.4, 0.0, 0.0, False, cc.hudControl, cs, cc,
                                        get_test_toggles(), lka_icon=2, lfa_icon=2)
    lkas_msgs = [msg for msg in msgs if msg[0] == 0x110]
    assert len(lkas_msgs) == 1
    ...
    assert parser.vl["LKAS_ALT"]["LKA_ICON"] == 2
    assert parser.vl["LKAS_ALT"]["LKAS_ANGLE_ACTIVE"] == 2
    assert parser.vl["LKAS_ALT"]["ADAS_ACIAnglTqRedcGainVal"] == pytest.approx(0.20)
    assert parser.vl["LKAS_ALT"]["ADAS_StrAnglReqVal"] == pytest.approx(-46.4)

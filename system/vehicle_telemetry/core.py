...
VEHICLE_TELEMETRY_DRIVING_INTERVAL_SECONDS = 60.0
VEHICLE_TELEMETRY_CHARGING_INTERVAL_SECONDS = 120.0
VEHICLE_TELEMETRY_PARKED_INTERVAL_SECONDS = 0.0
TELEMETRY_MODES = ("off", "send", "galaxy")
...
def _normalize_vehicle_telemetry_config(raw):
  ...
  if not requested_mode:
    requested_mode = "send" if push_raw.get("enabled") else (_legacy_fetch_mode if fetch_raw.get("enabled") else "off")
  ...
  config["push"] = {
    "enabled": config["mode"] == "send" and bool(push_url and len(push_token) >= 32),
    "url": push_url,
...
def save_vehicle_telemetry_config(raw, path=None):
  ...
  if config["mode"] == "off":
    config_path.with_name(VEHICLE_TELEMETRY_CACHE_FILENAME).unlink(missing_ok=True)
    config_path.with_name(VEHICLE_TELEMETRY_STATUS_FILENAME).unlink(missing_ok=True)
  return config
...
def build_vehicle_telemetry_snapshot(car_state, timestamp=None, vehicle_fingerprint="", source_name=None, vin=""):
  ...
  payload = {
    ...
    "vin": str(vin or "").strip().upper() if re.fullmatch(r"[A-HJ-NPR-Z0-9]{17}", str(vin or "").strip(), re.IGNORECASE) else None,
    ...
    "minutesToFull": int(round(charging_time_remaining / 60.0))
    if 0.0 < charging_time_remaining < 7 * 24 * 60 * 60 else None,
...
def telemetry_response(snapshot, now=None, vehicle_id=""):
  ...
  identifier = str(vehicle_id or snapshot.get("vin") or "").strip()[:128]
  if identifier:
...
class VehicleTelemetryPublisher:
  ...
  def _post(self, push_config, snapshot):
    ...
    vehicle_id = str(snapshot.get("vin") or snapshot.get("vehicleFingerprint") or "").strip()
  ...
  def _worker(self):
    ...
    while True:
      ...
      if activity == "parked":
        self._wait(30.0)
        continue
      interval = (VEHICLE_TELEMETRY_CHARGING_INTERVAL_SECONDS if activity == "charging"
                  else VEHICLE_TELEMETRY_DRIVING_INTERVAL_SECONDS)
      ...
      should_push = first_push or transition or periodic_due

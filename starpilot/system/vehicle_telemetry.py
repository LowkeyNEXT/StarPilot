#!/usr/bin/env python3
"""Normalized vehicle-energy telemetry, persistent cache, and optional exporter.

Vehicle-specific CAN decoding belongs in opendbc. This module only consumes the
generic CarState fields and deliberately runs outside the driving control loop.
"""
...
def build_vehicle_telemetry_snapshot(car_state, timestamp=None, vehicle_fingerprint=""):
  ...
  # value prevents an unsupported or not-yet-observed vehicle from looking valid.
...
class VehicleTelemetryPublisher:
  ...
  def _config(self):
    return load_vehicle_telemetry_config(self.config_path) if self.config_path is not None else load_vehicle_telemetry_config()
    ...
    try:
      response = self.session.post(
        ...
        json={
          ...
          "sentAt": int(time.time() * 1000),  # noqa: TID251 - transport protocol uses Unix milliseconds
          "telemetry": telemetry,
        },
        timeout=(3.0, 5.0),
  ...
  def _write_status(self, success, status_code, push_config, activity):
    ...
    _atomic_write_json(self.status_path, {
      "schemaVersion": VEHICLE_TELEMETRY_SCHEMA_VERSION,
      "updatedAt": time.time(),  # noqa: TID251 - persisted diagnostics use wall-clock time

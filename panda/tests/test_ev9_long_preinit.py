...
def test_firmware_usb_gate_blocks_disruptive_mutations_until_release():
  ...
  assert lpp.ev9_test_usb_request_allowed(0xDC, SAFETY_HYUNDAI_CANFD, 0x8495)
  assert lpp.ev9_test_usb_request_allowed(0xDC, SAFETY_HYUNDAI_CANFD, 0x8C95)
  assert lpp.ev9_test_usb_request_allowed(0xE7, 0, 0)
  assert lpp.ev9_test_preserve_can_on_safety_transition(SAFETY_HYUNDAI_CANFD, 0x8495)
...
def test_host_watchdog_loss_after_handoff_does_not_restart_resident_bridge():
  enter_handoff()
  assert lpp.set_safety_hooks(SAFETY_HYUNDAI_CANFD, 0x8495) == 0
...
def test_fdcan_restore_drain_checks_and_cancels_hardware_pending():
  ...
  llfdcan = (Path(__file__).parents[1] / "board" / "stm32h7" / "llfdcan.h").read_text()
  ...
  radar_reset = integration.index("llcan_configure_in_init(radar, radar_number)", ecan_clear)
  ecan_reset = integration.index("llcan_configure_in_init(ecan, ecan_number)", radar_reset)
  ...
  shared_config = llfdcan.index("static void llcan_configure_in_init(FDCAN_GlobalTypeDef *FDCANx, uint32_t can_number)")
  base_init = llfdcan.index("bool llcan_init(FDCAN_GlobalTypeDef *FDCANx)", shared_config)
  assert "FDCANx->RXF0C" in llfdcan[shared_config:base_init]
  assert "FDCANx->TXBC" in llfdcan[shared_config:base_init]
  assert "llcan_configure_in_init(FDCANx, can_number)" in llfdcan[base_init:]
  assert "FDCANx->RXF0C" not in integration
  assert "FDCANx->TXBC" not in integration
  ...
  policy = (Path(__file__).parents[1] / "board" / "ev9_long_preinit_can.h").read_text()
  assert "ev9_long_preinit_internal_tx_allowed(to_push, bus_number)" in policy
  assert "ev9_long_preinit_prepare_host_tx(to_push, bus_number, skip_tx_hook)" in policy
  assert "can_push(" not in policy
  assert can_common.count("queued = can_push(can_queues[bus_number], to_push)") == 1
  assert "firmware_feature_can_tx_action(to_push, bus_number, skip_tx_hook, internal)" in can_common

...
static ev9_preinit_replay_t *ev9_preinit_find_replay(uint16_t addr, uint8_t len) {
  ev9_preinit_replay_t *match = NULL;
  for (uint8_t i = 0U; (i < EV9_PREINIT_REPLAY_COUNT) && (match == NULL); i++) {
    if ((ev9_preinit_replay[i].addr == addr) &&
        ((len == 0U) || (ev9_preinit_replay[i].len == len))) {
      match = &ev9_preinit_replay[i];
    }
  }
  return match;
}
...
static bool ev9_preinit_force_neutral_addr(uint16_t addr) {
  const ev9_preinit_replay_t *replay = ev9_preinit_find_replay(addr, 0U);
  return (replay != NULL) && ((replay->flags & EV9_PREINIT_FRAME_FORCE_NEUTRAL) != 0U);
}

static bool ev9_preinit_slow_claim_addr(uint16_t addr) {
  const ev9_preinit_replay_t *replay = ev9_preinit_find_replay(addr, 0U);
  return (replay != NULL) && ((replay->flags & EV9_PREINIT_FRAME_SLOW_CLAIM) != 0U);
}
...
static bool ev9_preinit_bridge_tx_allowed(const CANPacket_t *packet) {
  ...
  const ev9_preinit_replay_t *replay = ev9_preinit_find_replay(packet->addr, GET_LEN(packet));
  if ((replay == NULL) || (packet->bus != replay->bus)) {
    return false;
  }
...
static const uint8_t *ev9_preinit_fallback(uint16_t addr) {
  const ev9_preinit_replay_t *replay = ev9_preinit_find_replay(addr, 0U);
  return (replay != NULL) ? replay->fallback : NULL;
}
...
static bool ev9_preinit_is_relevant_canfd_tuple(const CANPacket_t *packet) {
  ...
  const uint8_t len = GET_LEN(packet);
  return ((packet->addr == 0x35U) && (len == 32U)) ||
         ((packet->addr == 0xA0U) && (len == 24U)) ||
         ((packet->addr == 0x175U) && (len == 24U)) ||
         (ev9_preinit_find_replay(packet->addr, len) != NULL);
}
...
static bool ev9_preinit_is_critical_adas(const CANPacket_t *packet) {
  ...
  const ev9_preinit_replay_t *replay = ev9_preinit_find_replay(packet->addr, GET_LEN(packet));
  return (replay != NULL) && (packet->bus == replay->bus) &&
         ((replay->flags & EV9_PREINIT_FRAME_CRITICAL) != 0U);
}

...
#define EV9_PREINIT_BSM_RIGHT_MASK 0x08U
...
static const uint8_t ev9_preinit_fallback_161[32] =
  "\x00\x00\x00\x01\x00\x00\x00\x00\xc0\xff\xf0\xc0\x03\x00\x00\x40\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\x00\x00\x00\x00\x00\x00";
...
static const uint8_t ev9_preinit_fallback_1ba[24] =
  "\x00\x00\x00\x00\x00\x00\x00\x88\x02\x00\x00\x00\x00\x00\x00\x00\x0f\x01\x00\x00\x00\x00\x00\x0f";
...
// Decoded 0x1BA signal bytes for no detection, detection, and detection with
// the matching blinker. These are the same deltas used by the host bridge.
static const uint8_t ev9_preinit_bsm_states[3][3][6] = {
  {{0x00U, 0x00U, 0x00U, 0x00U, 0x80U, 0x00U},
    {0x01U, 0x01U, 0x00U, 0x20U, 0x88U, 0x04U},
    {0x01U, 0x02U, 0x00U, 0x40U, 0x90U, 0x08U}},
  {{0x41U, 0x00U, 0x02U, 0x00U, 0x88U, 0x01U},
    {0x41U, 0x01U, 0x02U, 0x20U, 0x88U, 0x05U},
    {0x41U, 0x02U, 0x02U, 0x40U, 0x90U, 0x09U}},
  {{0x81U, 0x00U, 0x04U, 0x00U, 0x90U, 0x02U},
    {0x81U, 0x01U, 0x04U, 0x20U, 0x90U, 0x06U},
    {0x81U, 0x02U, 0x04U, 0x40U, 0x90U, 0x0AU}},
};
...
static bool ev9_preinit_left_blindspot = false;
static bool ev9_preinit_right_blindspot = false;
static bool ev9_preinit_left_blinker = false;
static bool ev9_preinit_right_blinker = false;
...
static void ev9_long_preinit_init(void) {
  ...
  ev9_preinit_left_blindspot = false;
  ev9_preinit_right_blindspot = false;
  ev9_preinit_left_blinker = false;
  ev9_preinit_right_blinker = false;
  ...
  if (valid_canfd_crc && (packet->bus == EV9_PREINIT_BUS_ECAN)) {
    for (uint8_t i = 0U; i < (sizeof(ev9_preinit_replay) / sizeof(ev9_preinit_replay[0])); i++) {
      if ((packet->addr == ev9_preinit_replay[i].addr) && (GET_LEN(packet) == ev9_preinit_replay[i].len)) {
        ...
        switch (packet->addr) {
          case 0x12AU: neutral_fallback = ev9_preinit_fallback_12a; break;
          case 0x160U: neutral_fallback = ev9_preinit_fallback_160; break;
          case 0x161U: neutral_fallback = ev9_preinit_fallback_161; break;
          case 0x162U: neutral_fallback = ev9_preinit_fallback_162; break;
          case 0x1A0U: neutral_fallback = ev9_preinit_fallback_1a0; break;
          case 0x1BAU: neutral_fallback = ev9_preinit_fallback_1ba; break;
          default: break;
        }
...
static void ev9_preinit_neutralize(CANPacket_t *packet) {
  if (packet->addr == 0x1A0U) {
    ...
    const uint8_t left_state = ev9_preinit_left_blindspot ? (ev9_preinit_left_blinker ? 2U : 1U) : 0U;
    const uint8_t right_state = ev9_preinit_right_blindspot ? (ev9_preinit_right_blinker ? 2U : 1U) : 0U;
    const uint8_t *bsm_state = ev9_preinit_bsm_states[left_state][right_state];
    packet->data[3] ^= bsm_state[0];
    packet->data[4] ^= bsm_state[1];
    packet->data[5] ^= bsm_state[2];
    packet->data[6] ^= bsm_state[3];
    packet->data[7] ^= bsm_state[4] ^ 0x80U;
    packet->data[16] ^= bsm_state[5];
  }
}
...
static void ev9_long_preinit_rx_hook(const CANPacket_t *packet, uint32_t now_us) {
  ...
  if (packet->fd && (packet->bus == EV9_PREINIT_BUS_ECAN) &&
      (packet->addr == 0x36AU) && (GET_LEN(packet) == 16U)) {
    const uint16_t checksum = (uint16_t)packet->data[0] | ((uint16_t)packet->data[1] << 8U);
    if (ev9_preinit_crc(packet) == checksum) {
      ev9_preinit_left_blindspot = (packet->data[3] & EV9_PREINIT_BSM_LEFT_MASK) != 0U;
      ev9_preinit_right_blindspot = (packet->data[3] & EV9_PREINIT_BSM_RIGHT_MASK) != 0U;
      ev9_preinit_last_blindspot_us = now_us;
    }
  }
  if ((packet->bus == EV9_PREINIT_BUS_ECAN) && (packet->addr == 0x3C1U) && (GET_LEN(packet) == 8U)) {
    ev9_preinit_left_blinker = (packet->data[3] & 0x40U) != 0U;
    ev9_preinit_right_blinker = (packet->data[4] & 0x01U) != 0U;
  }
...
static void ev9_preinit_service_bridge(uint32_t now_us) {
  ...
  if ((ev9_preinit_last_blindspot_us != 0U) &&
      (get_ts_elapsed(now_us, ev9_preinit_last_blindspot_us) > EV9_PREINIT_BLINDSPOT_TIMEOUT_US)) {
    ev9_preinit_left_blindspot = false;
    ev9_preinit_right_blindspot = false;
  }

  // Until host handoff, the returned tester-present frame also advertises that
  // Panda owns the successful knockout. Match the host's normal 1 Hz cadence.

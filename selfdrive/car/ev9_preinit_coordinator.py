...
class EV9PreinitCoordinator:
  ...
  def resolve_preinit_startup_cache(self, cached_params):
    ...
    if self.preinit_startup_enabled(cached_params):
      # CarParamsCache is decoded by the stock card path as a reader. The
      # direct preinit interface keeps that object as CI.CP, while normal card
      # startup mutates CP after interface construction. Detach only this
      # opt-in path into a builder; persistent recovery already returns one.
      as_builder = getattr(cached_params, "as_builder", None)
      return as_builder() if as_builder is not None else cached_params
    if not ev9_panda_preinit_armed(self.params):

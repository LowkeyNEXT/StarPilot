...
def test_alternate_startup_requires_every_gate_and_exact_cached_platform():
  coordinator = coordinator_module.EV9PreinitCoordinator()
  cached = SimpleNamespace(
    brand="hyundai", carFingerprint="KIA_EV9",
  ...
  coordinator.params = FakeParams(enabled)
  assert coordinator.preinit_startup_enabled(cached)

  for key in enabled:
    coordinator.params = FakeParams(enabled | {key: False})
    assert not coordinator.preinit_startup_enabled(cached)
  ...
  for change in (
    ...
    assert not coordinator.preinit_startup_enabled(SimpleNamespace(**(vars(cached) | change)))


def test_preinit_recovers_persistent_cache_after_manager_clears_ephemeral_cache():
  persistent = car.CarParams.new_message()
  persistent.brand = "hyundai"
  persistent.carFingerprint = "KIA_EV9"
  persistent.openpilotLongitudinalControl = True
  persistent.pcmCruise = False
  enabled = {
    "EV9LongPreinitPanda": True,
    "OpenpilotEnabledToggle": True,
    "AlphaLongitudinalEnabled": True,
  ...
  recovered = coordinator.resolve_preinit_startup_cache(None)

  assert recovered is not None
  assert recovered.carFingerprint == "KIA_EV9"
  assert coordinator.preinit_startup_enabled(recovered)


def test_preinit_detaches_ephemeral_cache_reader_for_normal_card_mutation():
  cached = car.CarParams.new_message()
  cached.brand = "hyundai"
  cached.carFingerprint = "KIA_EV9"
  cached.openpilotLongitudinalControl = True
  cached.pcmCruise = False
  coordinator = coordinator_module.EV9PreinitCoordinator()
  coordinator.params = FakeParams({
    "EV9LongPreinitPanda": True,
    "OpenpilotEnabledToggle": True,
    "AlphaLongitudinalEnabled": True,
  })

  with car.CarParams.from_bytes(cached.to_bytes()) as reader:
    resolved = coordinator.resolve_preinit_startup_cache(reader)

  resolved.alternativeExperience = 32
  assert resolved.alternativeExperience == 32


def test_preinit_cache_recovery_is_inert_when_toggle_is_off():
  persistent = car.CarParams.new_message()
  persistent.brand = "hyundai"
  persistent.carFingerprint = "KIA_EV9"
  persistent.openpilotLongitudinalControl = True
  persistent_raw = persistent.to_bytes()
  original = SimpleNamespace(carFingerprint="MOCK")
  coordinator = coordinator_module.EV9PreinitCoordinator()
  coordinator.params = FakeParams({"CarParamsPersistent": persistent_raw})

  assert coordinator.resolve_preinit_startup_cache(original) is original
  with car.CarParams.from_bytes(persistent_raw) as reader:
    assert coordinator.resolve_preinit_startup_cache(reader) is reader


def test_disabled_preinit_runtime_hooks_are_inert():
  coordinator = coordinator_module.EV9PreinitCoordinator()
  coordinator.ev9_preinit_enabled = False
  CS = SimpleNamespace(adasUnavailable=True)

  coordinator.update_runtime_state(CS, None)
  coordinator.collect_claim_receipts_from_can_list([])

  assert CS.adasUnavailable
  assert not coordinator.handle_control_step(CS, initialized=False)


def test_generic_card_delegates_preinit_startup_selection():
  card_source = Path(__file__).parents[1].joinpath("card.py").read_text()
  assert "if self.preinit_startup_enabled(cached_params):" in card_source
  assert "cached_params.carFingerprint == CAR.KIA_EV9" not in card_source
...
def test_confirmed_handoff_leaves_safety_rejections_to_normal_panda_handling(monkeypatch):
  ...
  card_instance.ev9_preinit_fault_history = SimpleNamespace(update=lambda _panda_states: True)
  card_instance._expected_ev9_panda_safety = lambda: ("hyundaiCanfd", 0xC95, 32)
...
def test_step_sends_nothing_before_fresh_claim(monkeypatch):
  ...
  for takeover_state in (EV9PreinitTakeoverState.OFF, EV9PreinitTakeoverState.WAIT_SAFETY,
    ...
    sent = []
    card_instance = Car.__new__(Car)
    card_instance.ev9_preinit_enabled = True
...
def test_controls_update_reuses_early_interface_initialization():
  ...
  card_instance.interface_initialized = True
  card_instance.CI = SimpleNamespace(init=lambda *_args: initialized.append(True))
...
def test_stale_cached_ev9_starpilot_safety_is_canonicalized():
  ...
  safety[0].safetyModel = car.CarParams.SafetyModel.hyundaiCanfd

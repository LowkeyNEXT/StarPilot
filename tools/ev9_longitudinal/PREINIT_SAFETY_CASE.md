...
| Misleading display | Panda ownership uses neutral warning/status frames. Enhanced objects, BSM, headway, and lane rendering belong to an independent optional dash feature and neutralize invalid or stale inputs. |
...
second message epoch. If the independent dash feature is installed, the composition hook seeds it from the same frozen
baseline; preinit otherwise continues its own basic neutral/status writer.
...
- host startup and warm-rearm tests pass independently of dash/BSM/object tests;

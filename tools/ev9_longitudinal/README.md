...
The EV9 uses the existing Panda `hyundaiCanfd` model with its CCNC angle-long specialization, the exact EV9 safety parameter,
and a narrow, length-specific TX allowlist. Other Hyundai CAN-FD flag combinations do not inherit the CCNC angle-long
reconstruction permissions. That specialization independently requires reconstruction `0x12A` to carry zero torque/assist request and requires
...
production `CarParams` profile is `0x8495`; a cached StarPilot profile may add
...
the CCNC angle-long `hyundaiCanfd` profile. This prevents an old incompatible CCNC/button cache from changing
the safety shape during resident handoff.

Route `00000180` exposed a mixed-firmware deployment: the base image was current while the selected HKG-remote image was
stale and still requested the obsolete dedicated safety model 36. Route `00000181` then proved that the original "route-backed" heartbeat identity

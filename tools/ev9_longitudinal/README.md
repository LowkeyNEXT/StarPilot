...
mandatory neutral ownership continuity and control to the normal host process.
...
Both variants compile Panda's standard H7 `main.c`. The EV9 module supplies compile-time ownership policy hooks, while
CAN queueing/receipts and FDCAN controller configuration remain shared base-driver code. Panda driver fixes therefore
flow into the variants without maintaining copied transmit or register-setup implementations.
...
| `HANDOFF` | The complete hardware-qualified host claim is one-way for this ignition epoch. Normal host/Panda safety behavior resumes; the basic continuity path keeps the ownership indication valid. |
...
## Host continuity and optional dash reconstruction
...
Preinit includes only the neutral frames required to maintain unambiguous ownership before and through host handoff. It
does not depend on enhanced dash reconstruction, object tracking, BSM heuristics, headway display, or vision input.

Dash reconstruction is a separate EV9-only feature. It also works after the normal Dom ignition-on knockout when
preinit is disabled or absent. When both features are installed, a small composition hook seeds the dash writer with
preinit's captured live counter baselines; neither feature imports or activates the other. Dash behavior is:
...
the runtime ownership epoch and host-receipt status, managed CAN bodies, cluster icon state, and matching route logs.
Detailed diagnostic transaction timing remains available from a direct raw Panda status read when debugging firmware;
it is intentionally not serialized into every route log.
